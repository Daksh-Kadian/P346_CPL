...
def mGaussJordan(A,B):
    n = len(B)
    for k in range(n):
        if abs(A[k][k]) < 1.0e-12: #to take care of the limitatoins with the float variable storage
 ...
 #d is the value by which N is increaased and iterations is the number of times N is be increased            
    ...
    while N < (iterations*d):
        ...
        for j in range(len(vars)):
            f.append(fx(vars[j]))
        for j in range(len(vars)):
            fsqr.append((fx(vars[j]))**2)
        sumf = sum(f)
        sumfsqr = sum(fsqr)
        F = ((b-a)/N)*sumf
        yaxis.append(F)
        SDf = math.sqrt((1/N)*sumfsqr - ((1/N)*sumf)**2)
        N = N + d
...
##################################################################################################################
#define a function for solving ODE using Forward Euler method
def deqForwardEuler(f, a, b, h, initial):
    t = []
...
##################################################################################################################
#define a function for solving ODE using Backward Euler method
def deqBackwardEuler(a, b, h, initial):
    t = []
    for i in range(math.ceil((b-a)/h)+1):
        t.append(a)
        a = a + h
    ...
    for n in range(len(t)-1):
        s[n+1] = deqEulerNewtonRaphson(s[n], t[n+1], h)
    return t, s



##################################################################################################################
#define a function for solving ODE using Newton Raphson mithod with Euler method
def deqEulerNewtonRaphson(y_old, x_new, h, tol=1e-12):
    """ Takes values y_old and x_new, and finds the root of the
    function f(y_new), returning y_new. """
    
    # initial guess:
    y_new = y_old
    f = fx(y_new, y_old, x_new, h)
    dfdx = dfxdx(y_new, x_new, h)
...
##################################################################################################################
#define a function for solving ODE using RK4
def deqRK4(F1, F2, a, b, h, y, u):
    xpoints = []
    ypoints = []
    upoints = []
